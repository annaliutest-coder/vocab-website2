// app.js - 網站版生詞分析助手（含分冊累積選擇、手動切分 & 合併功能 & 循環定位功能）
...
// 【新增】已知詞彙庫 (此清單內的詞絕對不會被切開)
// 包含使用者指定的預設詞 + 課本所有詞彙
let knownWords = new Set([
    "紅色", "綠色", "藍色", "黃色", "白色", "黑色", // 顏色保護
    "護龍", "三合院", "四合院", // 特殊名詞保護
    "還都", "看書", "吃飯", "一定", "什麼", "怎麼樣"
]); 
...
function analyzeText() {
  ...
  if (useAdvanced && typeof advancedSegment !== 'undefined') {
    // 【修改】將「已知詞彙庫」(含課本詞) 視為高優先級字典
    // 傳給 segmenter，確保這些詞被優先匹配
    const segmentDict = { ...tbclData };
    knownWords.forEach(w => {
        if (!segmentDict[w]) segmentDict[w] = '0'; // 若 TBCL 沒有，暫定 Level 0
    });

    // 呼叫新版 advancedSegment (會執行上下文歧義修正)
...
function displayResults() {
  ...
  if (results.length === 0) {
    ...
    results.forEach((item, index) => {
      ...
      div.title = `點擊搜尋「${item.word}」在文中的位置 (再次點擊可找下一個)`;
